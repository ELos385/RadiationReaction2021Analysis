...
def blade_z(s):
    ...
    """
    h = np.nan
    if s>=1 and s<=5: h = 18.0
    if s>=6 and s<=10: h = 17.5
    if s>=11 and s<=15: h = 17.0
    if s>=16 and s<=20: h = 16.5
    if s>=21 and s<=25: h = 16.0
    if s>=26 and s<=30: h = 15.5
    if s>=31 and s<=39: h = 15.0
    if s>=40 and s<=44: h = 18.4
    if s>=45 and s<=49: h = 18.4
    if s>=50 and s<=54: h = 18.3
    if s>=55 and s<=59: h = 18.2
    if s>=60: h = 18.1
    return h
...
for s in shot_num:
    ...
    #plt.plot(x, i, color=colors[z_idx % 10])
    gd = [18.4, 17.5, 17.0, 16.5, 16.0, 15.5, 15.0]
    if z in gd:
        plt.plot(x, i, color=colors[z_idx % 10])
