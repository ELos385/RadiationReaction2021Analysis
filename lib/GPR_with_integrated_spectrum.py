...
class GaussianProcessRegressorIntegratedY(MultiOutputMixin, RegressorMixin, BaseEstimator):
    ...
    def __init__(
        ...
        *,
        ...
        self.integrand_weight=integrand_weight
        self.kernel = kernel
        self.alpha = alpha
    ...
    def fit(self, X, y):
        ...
        K_integrated=np.zeros((len(self.y_train_), len(self.y_train_)))
        for i in len(self.y_train_):
            k_integrated_ax1=np.trapz(K[i, :]*self.integrand_weight[i, :], self.X_train_)
            for j in len(self.y_train_):
                K_integrated[i, j]=np.trapz(k_integrated_ax1[j]*self.integrand_weight[j, :], self.X_train_)

        K_integrated[np.diag_indices_from(K_integrated)] += self.alpha
        try:
            self.L_ = cholesky(K_integrated, lower=GPR_CHOLESKY_LOWER, check_finite=False)
    ...
    def predict(self, X, return_std=False, return_cov=False):
        ...
        if not hasattr(self, "X_train_"):  # Unfitted;predict based on GP prior
            if self.kernel is None:
                ...
                kernel = self.kernel
            dN_dE_mean = np.zeros(X.shape[0])
            if return_cov:
                dN_dE_cov = kernel(X)
                return dN_dE_mean, dN_dE_cov
            elif return_std:
                dN_dE_var = kernel.diag(X)
                return dN_dE_mean, np.sqrt(dN_dE_var)
            else:
                return dN_dE_mean
            ...
            dN_dE_mean=K_trans
            # dN_dE_mean = K_trans @ self.alpha_
            #
            # # undo normalisation
            # dN_dE_mean = self._y_train_std * dN_dE_mean + self._y_train_mean
    ...
    def log_marginal_likelihood(
        ...
        print("kernel shape =%s"%([K.shape]))
        print("len y_train = %s"%(len(self.y_train_)))
        K_integrated=np.zeros((len(self.y_train_), len(self.y_train_)))
        for i in len(self.self.X_train_):
            k_integrated_ax1=np.trapz(K[i, :]*self.integrand_weight[0, :], self.X_train_)
            for j in len(self.self.X_train_):
                K_integrated[0, 0]=np.trapz(k_integrated_ax1*self.integrand_weight[0, :], self.X_train_)
        dx=self.X_train_[1]-self.X_train_[0]
        print("K_integrated[0, 0] = %s"%K_integrated[0, 0])
        print("2d sum K *dx^2 = %s"%(np.sum(np.sum(K*self.integrand_weight[0, :], axis=1), axis=0)*dx**2))
        # Alg. 2.1, page 19, line 2 -> L = cholesky(K + sigma^2 I)
        K_integrated[np.diag_indices_from(K_integrated)] += self.alpha
        try:
            L = cholesky(K_integrated, lower=GPR_CHOLESKY_LOWER, check_finite=False)
        ...
        log_likelihood_dims -= K_integrated.shape[0] / 2 * np.log(2 * np.pi)
